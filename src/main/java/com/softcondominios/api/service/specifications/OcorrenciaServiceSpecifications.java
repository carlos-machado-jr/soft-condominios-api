...
public class OcorrenciaServiceSpecifications {
	@Transactional
	public Specification<OcorrenciaDomain> searchBy(Boolean status, Long condominio) {
		...
		return (root, query, builder) -> {
			...
			if(Objects.nonNull(condominio)) {
				Path<CondominioDomain> condominioCampo = root.get("morador").get("condominio");
